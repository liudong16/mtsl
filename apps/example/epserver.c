...
thread_context_t g_ctxs[MAX_CPUS];
int core_limit;
static int turn = 0;
/*----------------------------------------------------------------------------*/
const char *www_main;
...
SendUntilAvailable(struct thread_context *ctx, int sockid, struct server_vars *sv)
{
	...
	while (ret > 0) {
		...
		ret = write1(sockid, fcache[sv->fidx].file + sv->total_sent, len);
		if (ret < 0) {
...
HandleReadEvent(struct thread_context *ctx, int sockid, struct server_vars *sv)
{
	...
	rd = read1(sockid, buf, HTTP_HEADER_LEN);
	if (rd <= 0) {
		return rd;
	}
    ...
    sent = write1(sockid, response, len);
...
CreateListeningSocket(struct thread_context *ctx)
{
	...
	listener = socket1(AF_INET, SOCK_STREAM, 0);
	if (listener < 0) {
	...
	*/
    ret = bind1(listener, (struct sockaddr *)&saddr, sizeof(struct sockaddr_in));
...
RunServerThread(void *arg)
{
    ...
    g_ctxs[turn] = ctx;
    turn++;
    printf("[CPU %d] server thread id is %lu.\n", core, pthread_self());
